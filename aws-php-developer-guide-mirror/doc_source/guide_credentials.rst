...
you follow the :iam-ug:`best-practices` when managing your credentials.
...
.. note::
    ...
    credentials by reading :STS-ug:`reference_aws-services-that-work-with-iam`.
    ...
    use cached instance profile credentials instead. For more information, see :doc:`guide_configuration`.

For more information, see :EC2-ug:`iam-roles-for-amazon-ec2`.
...
.. note::
    ...
    the AWS Security Token Service (AWS STS) are not supported by every
    service. Please check if the service you are using supports temporary
    credentials by reading :iam-ug:`reference_aws-services-that-work-with-iam`.

For more information, see :EC2-ug:`task-iam-roles`.
...
through third-party identity providers (read more about :IAM-ug:`Web Identity Federation
...
your application or project, see :STS-ug:`<STSUseCases>` in the AWS STS documentation.
